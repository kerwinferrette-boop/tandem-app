...
const EXERCISE_BANK = {
  ...
  'db-lateral-raise':{
    ...
    muscleGroups:{primary:['lateral_delt'],secondary:['supraspinatus','anterior_delt','upper_trap']},
  ...
  'single-arm-cable-lateral-raise':{
    ...
    muscleGroups:{primary:['lateral_delt'],secondary:['supraspinatus','anterior_delt','upper_trap','core']},
    emphasis:['shoulders','upper_body'], equipment:'cable', tier:'full_gym', category:'isolation', oneRmFactor:null,
  ...
  'landmine-press':{
    ...
    muscleGroups:{primary:['anterior_delt'],secondary:['tricep','serratus_anterior']},
  ...
  'shrug-barbell':{
    ...
    muscleGroups:{primary:['upper_trap'],secondary:['middle_trap']},
  ...
  'tricep-rope-pushdown':{
    ...
    muscleGroups:{primary:['tricep_lateral','tricep_medial'],secondary:['tricep_long_head']},
  ...
  'tricep-overhead-extension':{
    ...
    muscleGroups:{primary:['tricep_long_head'],secondary:['tricep_lateral','tricep_medial']},
  ...
  'skull-crusher':{
    ...
    muscleGroups:{primary:['tricep_long_head','tricep_medial'],secondary:['tricep_lateral']},
  ...
  'tricep-dip-machine':{
    ...
    muscleGroups:{primary:['tricep_lateral','tricep_medial','tricep_long_head'],secondary:['anterior_delt']},
  ...
  'leg-extension':{
    ...
    muscleGroups:{primary:['quad_rectus_femoris','quad_vastus_medialis','quad_vastus'],secondary:['quad_vastus_lateralis']},
  ...
  'sissy-squat':{
    ...
    muscleGroups:{primary:['quad_rectus_femoris','quad_vastus_lateralis','quad_vastus_medialis','quad_vastus'],secondary:['hip_flexor','core']},
  ...
  'lying-leg-curl':{
    ...
    muscleGroups:{primary:['hamstring_biceps_femoris'],secondary:['hamstring_semitendinosus','hamstring_semimembranosus']},
  ...
  'seated-leg-curl':{
    ...
    muscleGroups:{primary:['hamstring_biceps_femoris','hamstring_semimembranosus'],secondary:['hamstring_semitendinosus']},
    emphasis:['hamstrings','lower_body'], equipment:'machine', tier:'full_gym', category:'isolation', oneRmFactor:null,
  ...
  'nordic-curl':{
    ...
    muscleGroups:{primary:['hamstring_biceps_femoris','hamstring_semimembranosus','hamstring_semitendinosus'],secondary:['glute_max']},
  ...
  'good-morning':{
    ...
    muscleGroups:{primary:['hamstring','erector_spinae','glute_max'],secondary:['adductor','core']},
  ...
  'hip-thrust':{
    ...
    muscleGroups:{primary:['glute_max'],secondary:['hamstring','adductor']},
  ...
  'abductor-machine':{
    ...
    muscleGroups:{primary:['glute_medius'],secondary:['glute_max']},
  ...
  'side-plank':{
    ...
    muscleGroups:{primary:['quadratus_lumborum','oblique_external','glute_medius'],secondary:['oblique_internal','transverse_abdominis','glute_max']},
    emphasis:['core'], equipment:'bodyweight', tier:'home', category:'core', oneRmFactor:null,
  ...
  'bird-dog':{
    ...
    muscleGroups:{primary:['erector_spinae','glute_max','transverse_abdominis'],secondary:['oblique_external','oblique_internal','hamstring']},
    emphasis:['core','full_body'], equipment:'bodyweight', tier:'home', category:'core', oneRmFactor:null,
  ...
  'cable-crunch':{
    ...
    muscleGroups:{primary:['rectus_abdominis'],secondary:['oblique_external','oblique_internal','transverse_abdominis']},
  ...
  'pallof-press':{
    ...
    muscleGroups:{primary:['transverse_abdominis','oblique_internal','oblique_external'],secondary:['rectus_abdominis','anterior_delt','glute_medius']},
    emphasis:['core'], equipment:'cable', tier:'full_gym', category:'core', oneRmFactor:null,
  ...
  'russian-twist':{
    ...
    muscleGroups:{primary:['oblique_external','oblique_internal'],secondary:['rectus_abdominis','transverse_abdominis','hip_flexor']},
    emphasis:['core'], equipment:'bodyweight', tier:'home', category:'core', oneRmFactor:null,
...
function getSingleDay(focus, opts = {}) {
  ...
  const select = (groups, cat, used, freshGroups = [], usedPatterns = null) => {
    ...
    const eligible = (e) => tierOk(e) && e.category === cat && groupsMatch(e, reqGroups) && !injuryBlocked(e.name) && !used.has(e.name);
    // See primaryOnlyMatch's declaration (shared, module-scope): a COMPOUND slot
    // must be trained by its requested muscle as the candidate's OWN primary
    // target, not merely a synergist — isolation/core/cardio slots are unaffected
    // (unchanged). SOFT, same guarantee D20 already makes ("soft, never hard-
    // excludes"): if the stricter pool would be EMPTY at this tier (verified:
    // 'anterior_delt'/'lateral_delt' has zero primary-tagged compound candidates
    // at 'home' tier — every dedicated shoulder press is machine/barbell/
    // full_gym), fall back to the original primary+secondary pool rather than
    // silently dropping the slot (D18's exact failure mode — BUG-82 — is what
    // this fallback exists to avoid).
    let pool = Object.values(EXERCISE_BANK).filter(e => eligible(e) && (cat !== 'compound' || primaryOnlyMatch(e, reqGroups)));
    ...
    // disturb) but before the primary-match/pattern-novelty tiebreaks below
    // (D29 forbids ranking by oneRmFactor here at all — see those tiebreaks).
    const coverageCount = (e) => reqGroups.filter(gr => primaryOnlyMatch(e, [gr])).length;
    return pool
      .sort((a, b) => {
...
function buildDynamicProgram(goal, days, weeks, sex, tier, emphasis, injuries, maxDb, rotation, experience, liftHistory, durationMinutes) {
  ...
  //
  // primaryOnlyMatch is function-scoped inside getSingleDay (above) and cannot be
  // read from here (buildDynamicProgram is a separate top-level function) — this is
  // the identical fix + rationale re-declared per BUG-82 (Kerwin, 2026-09-14 —
  // "two pressing sets back to back makes no sense"), not a design choice to keep.
  const primaryOnlyMatch = (e, groups) => groupsMatch({ muscleGroups: { primary: e.muscleGroups.primary, secondary: [] } }, groups);
  const bank = ({groups, cat, excl=[]}) => {
    ...
    // See primaryOnlyMatch's declaration (shared, module-scope — no second copy
    // to drift, the hazard D19 had to police for groupsMatch). SOFT fallback —
    // see identical rationale on getSingleDay's select() above: never let the
    // stricter primary-only compound filter silently empty a slot (D18/BUG-82).
    // Verified: at 'home' tier this fallback IS live (0 primary-tagged shoulder-
    // press compounds exist below full_gym), so removing it would silently drop
    // Day 1's secondary compound slot for every home-tier user.
    ...
    // eligibility stage, but nothing before this counted how many of the slot's
    // OWN requested muscles a candidate actually covers — Barbell Hip Thrust
    // (primary=glute_max only) beat Romanian-Deadlift-class candidates for that
    // exact slot, orphaning hamstring at 0 weekly volume for the whole day-count
    ...
    // ABOVE primary-match: covering more of what the slot actually asked for
    // outranks which single muscle is the "best" match, which is what Kerwin's
    // council-verdict ask ("synergies... properly attributed") means at the
    // single-slot level.
    ...
    return pool.sort((a, b) => {
        ...
        if (pm !== 0) return pm;
        // NO oneRmFactor RANK HERE — removed by council ruling R1, enforced by D29.
        // See the comment above bank()'s declaration for why, and DOCTRINE.md D29.
        // BUG-108/BUG-94 tiebreak — see the comment above getSingleDay's declaration.
        const eqDiff = equipmentAvailabilityRank(a) - equipmentAvailabilityRank(b);
        if (eqDiff !== 0) return eqDiff;
