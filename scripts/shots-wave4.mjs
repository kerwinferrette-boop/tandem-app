#!/usr/bin/env node
// shots-wave4.mjs — Wave 4 DoD screenshots (You tab: empty, populated, female map)
// at 390x844, saved to docs/screenshots/wave4/. Same serving/stub recipe as
// bug128-bottomnav-pin-smoke.mjs (zero network egress).
...
const OUT = path.join(ROOT, 'docs', 'screenshots', 'wave4');
...
async function main() {
  ...
  try {
    ...
    // 1. You tab, empty state (top)
    await page.evaluate(() => goTab('you'));
    await page.waitForTimeout(400);
    await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }));
    await page.waitForTimeout(150);
    await page.screenshot({ path: path.join(OUT, 'you-empty-top.png') });

    // 2. You tab, empty heatmap (scrolled)
    await page.evaluate(() => {
      const el = document.getElementById('youMuscleMap');
      window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 90, behavior: 'instant' });
    });
    await page.waitForTimeout(200);
    await page.screenshot({ path: path.join(OUT, 'you-heatmap-empty.png') });

    // 3. Populated: seed a training week, re-render
    await page.evaluate(() => {
      const today = new Date().toISOString().slice(0, 10);
      const mk = (name, n) => Array.from({ length: n }, () => ({ name, weight: 100, reps: 8, done: true }));
      LS.set('tandem_history', [{ date: today, exercises: {
    ...
    await page.waitForTimeout(200);
    await page.screenshot({ path: path.join(OUT, 'you-heatmap-week.png') });

    // 4. Female art, same volume
    await page.evaluate(() => { cfg.sex = 'F'; renderMuscleHeatmap(); });
    await page.waitForTimeout(200);
    await page.screenshot({ path: path.join(OUT, 'you-heatmap-week-female.png') });

    if (jsErrors.length) throw new Error('JS errors: ' + jsErrors.join(' | '));
    console.log('wave4 shots written to', OUT);
  } finally {
    await browser.close();
    server.close();
  }
