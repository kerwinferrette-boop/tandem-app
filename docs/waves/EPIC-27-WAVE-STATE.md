...
- [ ] **Slice 5 — SUPERSEDED 2026-09-08, not built here.** The council verdict below (2026-09-07)
      was Kerwin's *first* pass and is kept for the paper trail, but he tightened it substantially
      the same day/next session — reject soft-only steering, actually resolve indirect volume
      rather than punt it, and build the mechanism app-wide (generated programs too, not just
      custom templates) rather than EPIC-27-only. That tightened version no longer fits inside one
      EPIC-27 slice — it is now its own Epic: **"Per-Muscle Volume Engine + Hard Block-Boundary
      Rebalancing"** (Notion `3d5ca37f935b8187a8becba1853fdf00`), which promotes D6b (was PENDING),
      closes the 2026-07-22 audit's Findings 3 and 4, and treats EPIC-27's custom-template renewal
      as one of two consumers (generated weekly programs being the other) rather than the whole
      feature. Foundational slice already seeded as its own tracker story:
      `D6b-per-muscle-volume-engine` (Notion `3d5ca37f935b8142b146c0ce57361679`).
      **This EPIC-27 slice's remaining job, once that Epic ships its engine:** wire the
      custom-template block-end (Slice 6's "block of any legal 4-12wk length ends", not a fixed
      week 13) to call that shared engine — a thin consumer, not a reimplementation.
      Original council verdict, superseded but preserved for context:
      ...
      notably reversed: the verdict's soft-steering-only stance (superseded, Kerwin wants the
      planning-layer decision hard) and its "don't resolve Finding 4 here" stance (superseded,
      Kerwin wants it actually resolved — see the new Epic's Agent Context Notes §B for the
      resolution, a per-muscle-differentiated MEV/MAV/MRV table, not a fractional-credit hack).
